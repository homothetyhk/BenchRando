...
namespace BenchRando.Rando
{
    ...
    {
        public static BenchRandomizationSettings Settings => BenchRandoMod.GS.BenchRandomizationSettings;
        public static List<string> RandomizedBenches;
        public static List<string> NonrandomizedBenches;
        public static List<string> Benches;
        public static Dictionary<string, string> ModifiedNearStyles;
        public static Dictionary<string, string> ModifiedFarStyles;
        ...
        {
            NonrandomizedBenches = null;
            RandomizedBenches = null;
            Benches = null;
            ModifiedNearStyles = null;
            ModifiedFarStyles = null;
            BRData.Reset();
        }

        public static void SetBenches(int seed)
        {
            Clear();
            BRData.Setup();
        ...
        public static void Finish(RandoController rc)
        {
            ...
            if (Settings.RandomizeBenchSpots)
            {
                ...
                foreach (string s in BRData.BenchLookup.Values.Where(b => b.IsBaseBench).Select(b => b.Name).Except(Benches))
                {
                ...
                foreach (BenchDef def in Benches.Select(b => BRData.BenchLookup[b]).Where(b => !b.IsBaseBench))
                {
                    IC.BenchDeployerGroup bdg = def.GetBenchDeployerGroup();
                    if (ModifiedNearStyles != null && ModifiedNearStyles.TryGetValue(def.Name, out string nearStyle))
                    {
                        bdg.BenchDeployer = bdg.BenchDeployer with { NearStyle = nearStyle };
                    }
                    if (ModifiedFarStyles != null && ModifiedFarStyles.TryGetValue(def.Name, out string farStyle))
                    {
